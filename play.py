...
root = Tk()
root.title("Music Player Software - {}".format(song_name))  # Set window title
...
# Load images
play_image = PhotoImage(file="playmusic_page/assets/play_icon.png")
pause_image = PhotoImage(file="playmusic_page/assets/pause_icon.png")
forward_image = PhotoImage(file="playmusic_page/assets/next_icon.png")
backward_image = PhotoImage(file="playmusic_page/assets/previous_icon.png")
forward_10s_image = PhotoImage(file="playmusic_page/assets/forward.png")
backward_10s_image = PhotoImage(file="playmusic_page/assets/backward.png")
...
song_name_label = Label(root, text="", font=("Arial", 38))
...
initial_image_path = "playmusic_page/assets/ATape_small.png"  # Replace with the actual path to your initial image
...
def rotate_image():
    rotation_angle = 0
    image_path = "playmusic_page/assets/ATape_small.png"  # Replace with the actual path to your image
...
def set_song_position(position):
    pygame.mixer.music.set_pos(position)  # Set the music position
is_completed = False
...
# Upload song name
def update_song_name_label():
    global current_music_index
    while True:
        song_filename = os.path.splitext(os.path.basename(music_files[current_music_index]))[0]
        song_name_label.config(text=song_filename)
        root.update()  # Update the Tkinter window to reflect changes
        time.sleep(1)  # Update the song name label every 1 second
...
def load_music():
    ...
    current_music = MP3(music_files[current_music_index])   # Load a new music file
    pygame.mixer.music.load(music_files[current_music_index])   # Load a new music file
    is_playing = False   # Reset the play/pause button to "play" status
    is_paused = False    # Reset the play/pause button to "play" status
    play_pause_button.config(image=play_image)
    duration_label.config(text="00:00/" + format_time(current_music.info.length))   # update duration tags
    current_position = 0   # Reset current position
    reset_progress_bar()   # Reset progress bar
    song_filename = os.path.splitext(os.path.basename(music_files[current_music_index]))[0]   # Update song title tags
    song_name_label.config(text=song_filename)   # Update song title tags
...
song_name_update_thread = threading.Thread(target=update_song_name_label)
song_name_update_thread.daemon = True  # Daemonize the thread so it stops when the main thread exits
song_name_update_thread.start()
