...
class RegisterFrame(tk.Toplevel):
    def __init__(self, login_frame, *args, **kwargs):
        ...
        self.login_frame = login_frame  # 保存登录页面的引用
...
class LoginFrame(tk.Frame):
    def __init__(self, master=None):
        ...
        self.bg_image = Image.open("login_page/assets/background.jpeg")
        ...
        self.center_image = Image.open("login_page/assets/background.jpeg")
    ...
    def create_widgets(self):
        # Load and display the background image
        bg_image = Image.open("login_page/assets/background.jpeg")
    ...
    def login(self):
        ...
        if not username.strip() or not password.strip():
            ...
            if username in self.user_data and self.user_data[username]["password"] == password:
                ...
                self.open_main_page(username)  # 跳转到主页面
                self.destroy()
                # 在登录成功后将用户名保存到文件中
            else:
